...
export default function History() {
  ...
  // GAP 6: Estado para controlar el modal de evidencia completa
    ...
    <div className="min-h-screen bg-gray-50 py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5xl mx-auto bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="px-6 py-5 border-b border-gray-200 bg-gray-900 flex justify-between items-center">
          ...
          <span className="text-xs bg-gray-800 text-gray-300 px-3 py-1 rounded-full font-mono">
                ...
                {reviews.map((review) => (
                  <tr key={review.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-6 py-4 text-right">
                      {/* GAP 6: Botón conectado para abrir el modal */}
                      <button 
      ...
      {/* --- GAP 6: MODAL DE EVIDENCIA COMPLETA --- */}
      {selectedReview && (
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-gray-100">
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-xl border border-gray-200 text-sm">
                <div>
                  <span className="text-gray-500 block text-xs uppercase font-semibold">Lenguaje</span>
                  <strong className="text-gray-800">{selectedReview.language}</strong>
                </div>
                <div>
                  <span className="text-gray-500 block text-xs uppercase font-semibold">Criterio</span>
                  <strong className="text-gray-800">{selectedReview.review_type}</strong>
                </div>
                <div>
                ...
                <div>
                  <span className="text-gray-500 block text-xs uppercase font-semibold">Fecha</span>
                  <strong className="text-gray-800">{new Date(selectedReview.created_at).toLocaleString()}</strong>
                </div>
              </div>
              ...
              {selectedReview.student_comment && (
                <div>
                  ...
                  <p className="text-sm bg-gray-100 text-gray-800 p-3 rounded-lg border border-gray-200 italic">
                    "{selectedReview.student_comment}"
                  </p>
                </div>
              )}
            ...
            <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end">
              <button 
                onClick={() => setSelectedReview(null)}
                className="px-5 py-2 bg-gray-900 hover:bg-gray-800 text-white text-sm font-bold rounded-lg transition cursor-pointer"
              >
                Cerrar
              </button>
            </div>
