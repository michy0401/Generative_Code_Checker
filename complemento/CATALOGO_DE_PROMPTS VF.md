...
Permitir la reevaluación de un código previamente analizado reinyectando el contexto completo de la revisión anterior almacenado en Supabase junto con la aclaración o nueva pregunta expresada por el usuario (`motivo_regeneracion`).
...
| Objeto / Campo | Tipo de Dato | Requerido | Descripción y Propósito Pedagógico |
|---|---|---|---|
| `summary.score` | integer | Sí | Puntuación cuantitativa de 0 a 100 calculada según la gravedad de los hallazgos. |
| `findings.category` | string | Sí | Restringido a enum: `"Error"`, `"Improvement"`, `"Recommendation"`. |
| `findings.severity` | string | Sí | Restringido a enum: `"High"`, `"Medium"`, `"Low"`. Indica la urgencia de solución. |
| `findings.line` | integer | Sí | Número de línea exacto dentro del código del estudiante donde se detecta el problema. |
| `explanation` | array | Sí | Lista de objetos vinculados mediante `finding_id` con desglose pedagógico: `why`, `impact`, `how_to_fix`. |
| `suggested_code` | object | Sí | Contiene `improved_code` (string) y `changes_summary` (array de strings con los cambios aplicados). |
| `tests` | array | Sí | Casos de prueba sugeridos con `title`, `description` y `expected_result` para promover la cultura de QA. |
| `warnings` | array | Sí | Advertencias sobre la naturaleza del análisis y alertas de seguridad. |

---

## 📊 7. Matriz de Evidencia de Pruebas de QA y Trazabilidad (5 Casos Reales en Supabase)
